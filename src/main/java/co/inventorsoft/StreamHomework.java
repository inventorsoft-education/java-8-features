...
public class StreamHomework {
    ...
    public List<Person> extractTeenagers(final List<Person> people) {
        return people.stream().filter(person -> person.getAge() >= 13 && person.getAge() <= 19).collect(Collectors.toList());
    }
    ...
    public List<User> createUsers(final List<String> emails) {
        return emails.stream().filter(s -> s != null).distinct().map(User::new).collect(Collectors.toList());
    }
    ...
    public Map<String, User> groupByEmail(final List<User> users) {
        ...
        return users.stream().collect(toMap(User::getEmail, Function.identity()));
    }
    ...
    public String collectDistinctNames(final List<Person> people) {
        return "Distinct names: " + people.stream().map(Person::getName).distinct().collect(Collectors.joining(", ")) + "!";
    }
}
