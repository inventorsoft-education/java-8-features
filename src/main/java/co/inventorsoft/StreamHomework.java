...
public class StreamHomework {
    ...
    public List<Person> extractTeenagers(final List<Person> people) {
        List<Person> personList = people.stream().filter(person -> person.getAge() >= 13 && person.getAge() <= 19).collect(Collectors.toList());
        return personList;
    }
    ...
    public List<User> createUsers(final List<String> emails) {
        List<User> userList = emails.stream().distinct().filter(Objects::nonNull).map(User::new).collect(Collectors.toList());
        return userList;
    }
    ...
    public Map<String, User> groupByEmail(final List<User> users) {
        Map<String, User> maps = users.stream().collect(Collectors.toMap(User::getEmail, Function.identity()));
        return maps;
    }
    ...
    public String collectDistinctNames(final List<Person> people) {
        String names = people.stream().map(Person::getName).distinct().collect(Collectors.joining(", "));
        return String.format("Distinct names: %s!", names);
    }
}
