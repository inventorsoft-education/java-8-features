...
public class StreamHomework {
    ...
    public List<Person> extractTeenagers(final List<Person> people) {
        return people.stream().filter(person -> person.getAge() >= 13 && person.getAge() <= 19).collect(toList());
    }
    ...
    public List<User> createUsers(final List<String> emails) {
        return emails.stream().distinct().filter(Objects::nonNull).map(User::new).collect(toList());
    }
